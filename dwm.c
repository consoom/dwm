		...
		if (text[i] == '^' && !isCode) {
			...
			while (text[++i] != '^') {
				if (text[i] == 'c') {
					...
					int c = atoi(text + ++i);
					drw_clr_create(drw, &drw->scheme[ColFg], sbcolor[c]);
				} else if (text[i] == 'B') {
					int c = atoi(text + ++i);
					drw_clr_create(drw, &drw->scheme[ColBg], sbcolor[c]);
				} else if (text[i] == 'd') {
					drw->scheme[ColFg] = scheme[SchemeNorm][ColFg];
					drw->scheme[ColBg] = scheme[SchemeNorm][ColBg];
				} else if (text[i] == 'r') {
...
focus(Client *c)
{
	if (!c || !ISVISIBLE(c)) {
		...
		if (!c) /* If there are no non-sticky clients; try to find sticky clients */
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        ...
        XRDB_LOAD_COLOR("dwm.color0",  sbcol0);
        XRDB_LOAD_COLOR("dwm.color1",  sbcol1);
        XRDB_LOAD_COLOR("dwm.color2",  sbcol2);
        XRDB_LOAD_COLOR("dwm.color3",  sbcol3);
        XRDB_LOAD_COLOR("dwm.color4",  sbcol4);
        XRDB_LOAD_COLOR("dwm.color5",  sbcol5);
        XRDB_LOAD_COLOR("dwm.color6",  sbcol6);
        XRDB_LOAD_COLOR("dwm.color7",  sbcol7);
        XRDB_LOAD_COLOR("dwm.color8",  sbcol8);
        XRDB_LOAD_COLOR("dwm.color9",  sbcol9);
        XRDB_LOAD_COLOR("dwm.color10", sbcol10);
        XRDB_LOAD_COLOR("dwm.color11", sbcol11);
        XRDB_LOAD_COLOR("dwm.color12", sbcol12);
        XRDB_LOAD_COLOR("dwm.color13", sbcol13);
        XRDB_LOAD_COLOR("dwm.color14", sbcol14);
        XRDB_LOAD_COLOR("dwm.color15", sbcol15);
      }
    }
  }
